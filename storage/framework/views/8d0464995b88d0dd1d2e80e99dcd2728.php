...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'Track Point'); ?></title>
    <script src="<?php echo e(asset('tailwind.js')); ?>"></script>
...
<body class="bg-[--bg-color] text-[--text-color] min-h-screen flex items-center justify-center"
    ...
    <!-- <div id="messageBox" class="absolute top-3 <?php echo e(request()->is('login') || request()->is('register') ? 'left-3' : 'left-20'); ?> flex flex-col space-y-3 z-50"> -->
    ...
    <div class="wrapper flex-1 flex flex-col h-screen relative">
        <div id="messageBox" class="absolute top-5 mx-auto flex items-center flex-col space-y-3 z-50 text-sm w-full select-none pointer-events-none">
            ...
            <?php if(session('error')): ?>
                <?php if(is_array(session('error'))): ?>
                    <?php $__currentLoopData = session('error'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $message): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        ...
        <?php if(Auth::user() && Auth::user()->theme === 'dark'): ?>
            <script>
                changeTheme();
            </script>
        <?php endif; ?>
...
<?php /**PATH E:\Softwares\TrackPoint\resources\views/layouts/app.blade.php ENDPATH**/ ?>